...
import { Pencil, Globe, Instagram, Phone, Mail, ArrowLeft } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <Card className="border-border">
            <CardContent className="p-4">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">ROAS Atual</p>
              <p className="mt-1 text-2xl font-extrabold">{latestMetric ? `${Number(latestMetric.roas).toFixed(1)}x` : "—"}</p>
              {latestMetric && previousMetric && (
                <p className="text-xs text-muted-foreground">{variation(Number(latestMetric.roas), Number(previousMetric.roas))} vs mês anterior</p>
              )}
            </CardContent>
          </Card>
          <Card className="border-border">
            <CardContent className="p-4">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Media Spend</p>
              <p className="mt-1 text-2xl font-extrabold">{latestMetric ? formatCurrency(Number(latestMetric.mediaSpend)) : "—"}</p>
            </CardContent>
          </Card>
          <Card className="border-border">
            <CardContent className="p-4">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Faturamento</p>
              <p className="mt-1 text-2xl font-extrabold">{latestMetric ? formatCurrency(Number(latestMetric.revenue)) : "—"}</p>
            </CardContent>
          </Card>
          <Card className="border-border">
            <CardContent className="p-4">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Contrato</p>
              <p className="mt-1 text-2xl font-extrabold">{daysRemaining !== null ? `${daysRemaining}d` : "—"}</p>
              <p className="text-xs text-muted-foreground">
                {daysRemaining !== null && daysRemaining > 0 ? "dias restantes" : daysRemaining !== null ? "expirado" : "sem contrato"}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="space-y-6">
            <Card className="border-border">
              ...
              <CardContent className="space-y-3">
                ...
                {client.instagram && (<div className="flex items-center gap-2 text-sm"><Instagram className="h-4 w-4 text-muted-foreground" /><span>{client.instagram}</span></div>)}
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">Plano</span><span className="font-medium">{client.plan?.name || "—"}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Plataforma</span><span className="font-medium">{client.ecommercePlatform}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Gestor</span><span className="font-medium">{client.trafficManager?.name || "—"}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Designer</span><span className="font-medium">{client.designer?.name || "—"}</span></div>
                </div>
              </CardContent>
            </Card>
            <Card className="border-border">
              <CardHeader>
                <CardTitle className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Faturas Recentes</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-2">
                    {client.invoices.map((inv) => (
                      <div key={inv.id} className="flex items-center justify-between rounded-lg border border-border p-3">
                        ...
                        <Badge variant={inv.status === "PAGO" ? "default" : inv.status === "ATRASADO" ? "destructive" : "outline"}>{inv.status}</Badge>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <Card className="border-border">
              <CardHeader>
                <CardTitle className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Histórico</CardTitle>
              </CardHeader>
              <CardContent>
                {client.interactions.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {client.interactions.map((interaction) => (
                      <div key={interaction.id} className="flex gap-3 border-l-2 border-border pl-4">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-muted-foreground">
                            {new Date(interaction.createdAt).toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" })}
                          </p>
                        </div>
                      </div>
                    ))}
